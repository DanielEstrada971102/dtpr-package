...
def analyze_genmuon_matches(ev: Event):
    ...
    if not hasattr(ev, "segments"):
        raise ValueError("Event does not have 'segments' they are required to analyze matches.")
    if not hasattr(ev, "genmuons"):
        raise ValueError("Event does not have 'genmuons' they are required to analyze matches.")
    if not hasattr(ev, "tps"):
        raise ValueError("Event does not have 'tps' they are required to analyze matches.")
